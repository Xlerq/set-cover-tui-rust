...
pub fn get_data(file_path: &str) -> Result<SetCoverData, String> {
    ...
    .split_whitespace()
    .map(|num| num.parse().map_err(|e| format!("Błąd parsowania liczby w zbiorze uniwersalnym: {}", e)))
    .collect::<Result<Vec<usize>, String>>()?;

    let min_val = *universe.iter().min().ok_or("Zbiór uniwersalny jest pusty")?;
    if min_val != 1 {
    ...
    for (i, line) in lines.enumerate() {
        ...
        if !line.starts_with("[[") || !line.ends_with(']') {
            return Err(format!("Linia {} musi być w formacie [[elementy] koszt]", i + 2));
        }

        let content = &line[1..line.len() - 1];
        let parts: Vec<&str> = content.split(']').collect();
        if parts.len() != 2 {
            return Err(format!("Linia {}: Nieprawidłowy format, oczekiwano [[elementy] koszt]", i + 2));
        }

        let elements_str = parts[0].trim().strip_prefix('[').ok_or(format!("Linia {}: Brak [ w elementach", i + 2))?;
        let elements: Vec<usize> = elements_str
        .split_whitespace()
        .map(|num| num.parse().map_err(|e| format!("Błąd parsowania elementu w linii {}: {}", i + 2, e)))
        .collect::<Result<Vec<usize>, String>>()?;
